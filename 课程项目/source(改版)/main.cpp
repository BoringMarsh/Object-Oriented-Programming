...
int main()
{
	...
	while (if_colorball) {  /*若为真，则游戏不退出*/
		...
		case COVER_RET_LOAD:
			...
			ret = CoverMessagebox.control(ps);  /*取对话框操作*/
		...
		case COVER_RET_HELP:
			...
			CoverMessagebox.control(ps);  /*取操作，使得用户可以完成点击“确定”键的动作*/
		...
		/*5、按下“高分榜”键*/
		case COVER_RET_HS:
			CoverMessagebox.print(BUTTON_ONE);  /*打印对话框*/
			FileManager.get_highscore();   /*从文件中读取高分*/
			FileManager.sort_highscore();  /*将读取到的高分进行排序*/
			FileManager.show_highscore();  /*显示排序后的高分*/
			CoverMessagebox.control(ps);   /*取操作，使得用户可以完成点击“确定”键的动作*/
			CoverMessagebox.erase(TitleScreen);  /*擦除对话框*/
			continue;
			break;
		...
		case COVER_RET_EXIT:
			...
			switch (CoverMessagebox.control(ps)) {  /*取对话框操作*/
			case CVMB_RET_YES:
		...
		while (if_game) {  /*若为真，则游戏进行，不退回标题画面*/
			...
			case GAME_RET_OPTIONS:
				...
				while (1) {
					RET = GameMessagebox.control(ps);  /*取对话框操作*/
			...
			case GAME_RET_QUIT:
				...
				switch (GameMessagebox.control(ps)) {  /*取对话框操作*/

				/*4.1、按下“是”键*/
				case GMMB_RET_YES:
					FileManager.record(Client, "QUIT");
			...
			case GAME_RET_FAILURE:
				FileManager.record(Client, "FAIL");
				Client.matrix_clear(Ball);  /*清空内部数组*/
				...
				GameMessagebox.control(ps);  /*取操作，使得用户可以完成点击“确定”键的动作*/
	...
	cct_cls();       /*清屏*/
#else
	const char str[] = "SOUNDS LIKE YOU MAMA BANG";
#endif
	return 0;
}