...
/***************************************************************************
  函数名称：matrix_waynum_copy
  功    能：将结构体数组src[]中的移动步数信息复制到结构体数组dst[]中
  输入参数：ball dst[]：待复制的结构体数组
            const ball src[]：来源结构体数组（只读）
  返 回 值：无
  说    明：无
***************************************************************************/
void player::matrix_waynum_copy(ball dst[], const ball src[])
{
	for (int i = 0; i < 81; i++)
		dst[i].waynum = src[i].waynum;
}

/***************************************************************************
  函数名称：matrix_waynum_compare
  功    能：比较两结构体数组b1[]和b2[]的移动步数信息
  输入参数：const ball m1[]：待比较的结构体数组1（只读）
			const ball m2[]：待比较的结构体数组2（只读）
  返 回 值：bool：两结构体数组中的移动步数信息是否完全相等
  说    明：无
***************************************************************************/
bool player::matrix_waynum_compare(const ball b1[], const ball b2[])
{
	bool ret = true;  /*返回值，先设为真*/

	for (int i = 0; i < 81; i++) {  /*逐个元素进行比较，若发现有一项值不等，返回值设为假并结束检查*/
		if (b1[i].waynum != b2[i].waynum) {
			ret = false;
			break;
		}
	}
...
/***************************************************************************
  函数名称：matrix_waynum_clear
  功    能：清空内部数组的移动步数信息
  输入参数：ball b[]：游戏内部的对象数组
  返 回 值：无
  说    明：无
***************************************************************************/
void player::matrix_waynum_clear(ball b[])
{
	for (int i = 0; i < 81; i++)
		b[i].waynum = -1;  /*将当前位置上的移动步数信息置为-1*/
}

/***************************************************************************
  函数名称：matrix_waymark_clear
  功    能：清空内部数组的路径标记
  输入参数：ball b[]：游戏内部的对象数组
  返 回 值：无
  说    明：无
***************************************************************************/
void player::matrix_waymark_clear(ball b[])
{
	for (int i = 0; i < 81; i++)
		b[i].waymark = false;  /*将当前位置上的路径标记置为假*/
}

/***************************************************************************
  函数名称：matrix_waycoord_clear
  功    能：清空内部数组的路径位置记录
  输入参数：ball b[]：游戏内部的对象数组
  返 回 值：无
  说    明：无
***************************************************************************/
void player::matrix_waycoord_clear(ball b[])
{
	for (int i = 0; i < 81; i++)
		way_coord[i] = -1;  /*将路径位置全部置为-1*/
}

/***************************************************************************
  函数名称：countway_single_scan
  功    能：统计数组元素coord周围四个元素的移动步数情况
  输入参数：ball b[]：游戏内部的对象数组
            const int coord：内部数组中对应元素的一维坐标（只读）
  返 回 值：无
  说    明：无
***************************************************************************/
void player::countway_single_scan(ball b[], const int coord)
{
	const int X = coord % 9;        /*将一维坐标转换为列数*/
	const int Y = (coord - X) / 9;  /*将一维坐标转换为行数*/

	/*检查上下左右四个元素，若该元素存在、位置为空且移动步数情况未统计，则进行统计，步数比当前位置元素的移动步数多1*/
	/*1、检查正上方元素*/
	if (Y && !ball::if_ball[coord - 9] && b[coord - 9].waynum == -1)
		b[coord - 9].waynum = b[coord].waynum + 1;

	/*2、检查正下方元素*/
	if (Y < 8 && !ball::if_ball[coord + 9] && b[coord + 9].waynum == -1)
		b[coord + 9].waynum = b[coord].waynum + 1;

	/*3、检查左侧元素*/
	if (X && !ball::if_ball[coord - 1] && b[coord - 1].waynum == -1)
		b[coord - 1].waynum = b[coord].waynum + 1;

	/*4、检查右侧元素*/
	if (X < 8 && !ball::if_ball[coord + 1] && b[coord + 1].waynum == -1)
		b[coord + 1].waynum = b[coord].waynum + 1;
}

/***************************************************************************
  函数名称：countway
  功    能：统计内部数组所有元素周围四个元素的移动步数情况
  输入参数：ball b[]：游戏内部的对象数组
			const int src_coord：内部数组中起点位置对应元素的一维坐标（只读）
  返 回 值：无
  说    明：无
***************************************************************************/
void player::countway(ball b[], const int src_coord)
{
	ball B[81];  /*结构体数组，用来在每次统计后与游戏内部数组进行比对*/
	b[src_coord].waynum = 0;  /*起点处的移动步数为0*/

	while (!matrix_waynum_compare(b, B)) {  /*当一轮统计过后，移动步数情况没有变化，则停止循环*/
		matrix_waynum_copy(B, b);  /*将内部数组的移动步数情况复制到B[]中*/
		
		for (int i = 0; i < 81; i++) {  /*遍历内部数组，若发现已经统计过的元素，则以其为中心继续统计*/
			if (b[i].waynum >= 0)
				countway_single_scan(b, i);
		}
	}
}
...
/***************************************************************************
  ...
  输入参数：ball b[]：游戏内部的对象数组
            const int src_coord：内部数组中起点位置对应元素的一维坐标（只读）
			const int dst_coord：内部数组中终点位置对应元素的一维坐标（只读）
  返 回 值：无
  说    明：1、本函数统计了内部数组所有元素周围四个元素的移动步数情况
            2、本函数从终点开始向起点寻找路径
            3、本函数在寻找的同时对路过的位置进行路径标记
			4、可行方向优先级：上>下>左>右
***************************************************************************/
void player::findway(ball b[], const int src_coord, const int dst_coord)
{
	countway(b, src_coord);       /*统计所有可移动到的位置到起点的最短步数*/
	b[dst_coord].waymark = true;  /*由于路径一定经过终点，故终点位置元素路径标记置为真*/
	ball* p = &b[dst_coord];      /*指向终点位置的指针*/

	for (int i = b[dst_coord].waynum; i > 0; i--) {  /*每一次循环寻找到路径的其中一步，步数由终点位置元素的移动步数来决定*/
		const int X = (p - b) % 9;      /*将一维坐标转换为列数*/
		const int Y = (p - b - X) / 9;  /*将一维坐标转换为行数*/

		/*检查上下左右四个元素，若该元素存在、移动步数比当前位置少1且不为零，
		则进行路径标记，并将当前位置记录到数组way_coord[]中，同时将指针移动到下一步*/
		/*1、检查正上方元素*/
		if (Y && (p - 9)->waynum == p->waynum - 1 && p->waynum) {
			(p - 9)->waymark = true;
			way_coord[i - 1] = p - b;
			p = p - 9;
			continue;
		...
		/*2、检查正下方元素*/
		if (Y < 8 && (p + 9)->waynum == p->waynum - 1 && p->waynum) {
			(p + 9)->waymark = true;
			way_coord[i - 1] = p - b;
			p = p + 9;
			continue;
		...
		/*3、检查左侧元素*/
		if (X && (p - 1)->waynum == p->waynum - 1 && p->waynum) {
			(p - 1)->waymark = true;
			way_coord[i - 1] = p - b;
			p = p - 1;
			continue;
		...
		/*4、检查右侧元素*/
		if (X < 8 && (p + 1)->waynum == p->waynum - 1 && p->waynum) {
			(p + 1)->waymark = true;
			way_coord[i - 1] = p - b;
			p = p + 1;
			continue;
...
/***************************************************************************
  函数名称：showway
  功    能：在最短路径查找完成的基础上，将其显示出来
  输入参数：const ball b[]：游戏内部的对象数组（只读）
  返 回 值：无
  说    明：无
***************************************************************************/
void player::showway(const ball b[])
{
	int color = 0;

	for (int i = 0; i < 81; i++) {
		if (!b[i].waynum && b[i].waymark)  /*取起点位置珠子的颜色*/
			color = b[i].color;
	}

	for (int i = 0; i < 81; i++) {  /*逐个从way_coord[]中读取位置并显示出来，完成后结束循环*/
		if (way_coord[i] < 0)
			break;

		const int X = way_coord[i] % 9;        /*将一维坐标转换为列数*/
		const int Y = (way_coord[i] - X) / 9;  /*将一维坐标转换为行数*/

		cct_showstr(10 * X + 4, 5 * Y + 2, "    ", color);
		cct_showstr(10 * X + 4, 5 * Y + 3, "    ", color);
		Sleep(100);  /*延时，控制速度*/
	}
}
...
/***************************************************************************
  ...
  说    明：无
***************************************************************************/
void player::eraseway(const ball b[])
{
	int color = 0;

	for (int i = 80; i >= 0; i--) {
		const int X = i % 9;        /*将一维坐标转换为列数*/
		const int Y = (i - X) / 9;  /*将一维坐标转换为行数*/

		if (b[i].waymark) {  /*若找到被路径标记的元素，则擦除痕迹*/
			cct_showstr(10 * X + 4, 5 * Y + 2, "    ", COLOR_WHITE);
			cct_showstr(10 * X + 4, 5 * Y + 3, "    ", COLOR_WHITE);
		}
	}
}
...
/***************************************************************************
  ...
  功    能：移动珠子
  输入参数：ball b[]：游戏内部的对象数组
            const int src_coord：珠子起点一维坐标（只读）
			const int dst_coord：珠子终点一维坐标（只读）
  返 回 值：无
  说    明：无
***************************************************************************/
void player::move(ball b[], const int src_coord, const int dst_coord)
{
	/*若起点位置有珠子、终点位置无珠子，则进行移动操作*/
	if (ball::if_ball[src_coord] && !ball::if_ball[dst_coord]) {
		const int dst_x = dst_coord % 9;            /*将终点一维坐标转换为列数*/
		const int dst_y = (dst_coord - dst_x) / 9;  /*将终点一维坐标转换为行数*/
		b[dst_coord].set(dst_x, dst_y, b[src_coord].color);  /*设置终点位置珠子的坐标和颜色*/
		ball::if_ball[dst_coord] = true;  /*终点位置的标志设为真*/

		b[src_coord].color = COLOR_WHITE;  /*起点位置珠子的颜色设为白色*/
		ball::if_ball[src_coord] = false;  /*起点位置的标志设为假*/

		b[src_coord].print();  /*打印起点位置的珠子（已设为假，打印空白）*/
		b[dst_coord].print();  /*打印终点位置的珠子*/

		b[src_coord].x = -1;  /*设置起点位置珠子的列数（已移走，设为-1）*/
		b[src_coord].y = -1;  /*设置起点位置珠子的行数（已移走，设为-1）*/
	}
}
	...
	while (loop) {
		...
		case CCT_MOUSE_EVENT:
			...
			case MOUSE_LEFT_BUTTON_CLICK:
				switch (if_select) {
				case true:  /*若选定了一个珠子，则进行下一步操作*/
					if (RET[0] < 0)  /*若鼠标不在格子上，则不进行任何操作*/
						continue;

					switch (ball::if_ball[RET[0]]) {
					case true:
						update_state(STATE_BLOCKED);  /*若鼠标在一个不为空的格子上，则更新状态*/

						if (*if_sound)  /*根据需要播放声音*/
							Beep(SOUND_LSI, 200);
						break;
					case false:
						findway(b, 9 * select_y + select_x, RET[0]);  /*寻找最短路径*/
						showway(b);   /*显示路径*/
						eraseway(b);  /*擦除路径*/
						matrix_waymark_clear(b);   /*清空路径标记*/
						matrix_waycoord_clear(b);  /*清空路径位置记录*/

						switch (b[RET[0]].waynum) {
						case -1:
							update_state(STATE_NOWAY);  /*若移动步数仍为-1，则无有效路径，更新状态*/
							matrix_waynum_clear(b);     /*清空移动步数信息*/
						...
						default:
							...
							move(b, 9 * select_y + select_x, RET[0]);  /*移动珠子*/
							scan(b);  /*扫描可消除的珠子*/
				...
				case false:
					...
					else if (!ball::if_ball[RET[0]] && !RET[1]) {  /*鼠标在一个空位上，则更新状态*/
