...
int highscore_compare(const void* p1, const void* p2);  //规定排序方法

//存储高分记录数
#define HIGHSCORE_NUM  1024
...
private:
	...
	int highscore[HIGHSCORE_NUM];  //高分
public:
	...
	void record(player& client, const char* end_state);  //记录一局游戏的结果
	void get_highscore();   //从文件中读取高分
	void show_highscore();  //显示排序后的高分
	void sort_highscore();  //将读取到的高分进行排序

	file_tools() :filename("save .txt")  //构造函数
	{
		for (int i = 0; i < HIGHSCORE_NUM; i++)  /*将存放读取到高分的数组元素全部初始化为-1（有可能一局游戏得分为0）*/
			highscore[i] = 0;
	}
};