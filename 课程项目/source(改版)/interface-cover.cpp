...
int cover::if_button(const int X, const int Y)
{
	...
	if (X >= HELP_COORD_X && X <= HELP_COORD_X + 11 && Y >= HELP_COORD_Y && Y <= HELP_COORD_Y + 2)  /*判断是否在“帮助”键上*/
		p[3] = true;
	if (X >= HS_COORD_X && X <= HS_COORD_X + 11 && Y >= HS_COORD_Y && Y <= HS_COORD_Y + 2)          /*判断是否在“高分榜”键上*/
		p[4] = true;
...
void cover::selective_default(const int selection)
{
	...
	case 4:
		...
		break;
	case 5:
		button_hs.act(DEFAULT);       /*“高分榜”键取消高亮*/
		break;
	case 6:
...
void cover::selective_activate(const int selection)
{
	...
	case 4:
		...
		break;
	case 5:
		button_hs.act(ACTIVATE);       /*“高分榜”键设置高亮*/
		break;
	case 6:
	...
	while (loop) {
		...
		if (ret == CCT_MOUSE_EVENT && maction == MOUSE_LEFT_BUTTON_CLICK) {
			...
			case 4:
				...
				break;
			case 5:
				result = COVER_RET_HS, loop = 0;       /*取返回值、结束循环*/
				break;
			case 6:
